...
export async function GET(req) {
    ...
    try {
        ...
        if (!wompiResponse.ok) {
            ...
            return NextResponse.json({ error: "Failed to retrieve transaction from Wompi", details }, { status: wompiResponse.status })
        }
        ...
        return NextResponse.json({
            ...
            message: data.status_message || (status === "APPROVED"
        ...
        return NextResponse.json({ error: "Unable to verify payment", details: error.message ?? "Unknown error" }, { status: 500 })
    }
}